...
class EventFirebaseService {
  ...
  static Future<List<EventModel>> getRecentEvents({
    ...
    print('✅ Loaded ${allEvents.length} events in ${duration.inMilliseconds}ms');
  ...
  static Future<Map<DateTime, List<EventModel>>> getEventsForMonth(DateTime month) async {
      ...
      fetchTasks.add(
        _eventsCollection
          ...
          if (snapshot.docs.isNotEmpty) {
            eventsByDate[normalizedDate] = snapshot.docs
                .map((doc) => EventModel.fromFirestore(doc))
                .toList();
          }
        }).catchError((e) {
  ...
  static Future<void> updateEvent(
      DateTime eventDate, String eventId, Map<String, dynamic> updates) async {
    try {
  ...
  static Future<void> toggleLike(
      DateTime eventDate, String eventId, String userEmail) async {
    try {
      final dateKey = DateFormat('yyyy-MM-dd').format(eventDate);
