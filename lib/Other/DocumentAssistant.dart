...
class DocumentAssistantScreen extends StatefulWidget {
  ...
  @override
  State<DocumentAssistantScreen> createState() => _DocumentAssistantScreenState();
}
...
class _DocumentAssistantScreenState extends State<DocumentAssistantScreen> {
  ...
  final List<String> _languages = ['English', 'ಕನ್ನಡ (Kannada)', 'हिंदी (Hindi)'];
  ...
  Future<void> _processDocument() async {
    ...
    try {
      ...
      if (extractedText.isEmpty) {
        throw Exception('No text found in image. Please try with a clearer image.');
      }

      setState(() => _extractedText = extractedText);

      // Step 2: Analyze with Gemini AI
      final analysis = await _analyzeWithGemini(extractedText, _selectedLanguage);
  ...
  Future<String> _extractTextFromImage(File imageFile) async {
    ...
    try {
      ...
      if (fullText.isEmpty) {
        ...
        final devanagariText = await devanagariRecognizer.processImage(inputImage);
  ...
  Future<Map<String, dynamic>> _analyzeWithGemini(String text, String language) async {
...
''';
  ...
  Map<String, dynamic> _createBasicAnalysis(String text, String language) {
    ...
    return {
      ...
      'important_notes': ['Keep all documents ready', 'Fill in capital letters'],
      ...
      child: Column(
        ...
        children: [
          ...
          if (_analysisResult!['purpose'] != null) ...[
            ...
            _buildInfoCard(_analysisResult!['purpose'], Icons.info_outline, AppColors.accent),
            const SizedBox(height: 20),
          ],
          ...
          if (_analysisResult!['step_by_step_guide'] != null) ...[
            ...
            ...(_analysisResult!['step_by_step_guide'] as List).asMap().entries.map((entry) {
  ...
  Widget _buildFieldCard(Map<String, dynamic> field) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: field['is_mandatory'] == true ? AppColors.primary : Colors.grey[300]!,
              ...
              children: [
                ...
                if (field['how_to_fill'] != null) ...[
                  Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Icon(Icons.edit_note, size: 18, color: AppColors.success),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                ...
                if (field['example'] != null) ...[
                  Container(
                    ...
                    child: Row(
                      children: [
                        const Icon(Icons.lightbulb_outline, size: 18, color: AppColors.success),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Text(
        ...
        actions: [
          PopupMenuButton<String>(
            icon: Row(
              ...
              children: [
                Text(_selectedLanguage.split(' ')[0], style: const TextStyle(fontSize: 14)),
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(
              width: double.infinity,
              child: ElevatedButton.icon(
                onPressed: _imageFile != null && !_isProcessing ? _processDocument : null,
                icon: _isProcessing
                    ? const SizedBox(
                        width: 20,
                        height: 20,
